#!/usr/bin/env pwsh
# Organize notebooks for DeepSeek Quantization Framework
...
Write-Host "DeepSeek Quantization Notebook Organizer" -ForegroundColor Cyan
Write-Host "===========================================" -ForegroundColor Cyan
Write-Host ""
...
# Organize notebooks
Write-Host "`nOrganize notebooks? This will:" -ForegroundColor Cyan
Write-Host "  1. Copy notebooks to the notebooks/ directory" -ForegroundColor Cyan
Write-Host "  2. Make backup copies in archive/notebooks/" -ForegroundColor Cyan
Write-Host "  3. Leave originals in place for reference" -ForegroundColor Cyan
Write-Host "[Y/n]: " -NoNewline -ForegroundColor Yellow
$organize = Read-Host

if ($organize -eq "" -or $organize -eq "Y" -or $organize -eq "y") {
    foreach ($notebook in $notebooks) {
    ...
    # Create an index README in the notebooks directory
...
"@
    ...
    foreach ($notebook in $notebooks) {
        $readmeContent += "`n- `$($notebook.Name)` - "
        
        # Try to extract title from notebook
        try {
            $notebookContent = Get-Content -Path $notebook.FullName -Raw | ConvertFrom-Json
            $cells = $notebookContent.cells
            
            $titleCell = $cells | Where-Object { $_.cell_type -eq "markdown" } | Select-Object -First 1
            if ($titleCell) {
                $titleMatch = [regex]::Match($titleCell.source[0], "^#\s+(.*)")
                if ($titleMatch.Success) {
                    $title = $titleMatch.Groups[1].Value
                    $readmeContent += "$title"
                }
                else {
                    $readmeContent += "Quantization notebook"
                }
            }
            else {
                $readmeContent += "Quantization notebook"
